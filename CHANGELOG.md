...
- **First administrator from the deployment.** `VAULT_SETUP_ADMIN_USERNAME` and
  `VAULT_SETUP_ADMIN_PASSWORD` (plus optional `VAULT_SETUP_ADMIN_EMAIL`) create
  the first administrator at startup, for app-store install forms and unattended
  deployments that cannot use browser registration. The administrator signs in
  and chooses storage in the browser. The variables are used once and never
  change an existing account.
- When the browser cannot create the first administrator, the setup page now
  says why, names the address PrintStash saw, and lists what to change. It
  previously showed a single "registration is disabled" message.
