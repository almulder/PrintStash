...
| `administration` | Dynamic OSS settings, setup, audit and operational inspection | `runtime_config`, `setup_bootstrap`, `setup_storage`, `audit`, `vault_audit`, `release_check` |
...
`api.session_cookie` and `api.setup_session`; first ownership (browser claim and
`VAULT_SETUP_ADMIN_*` provisioning at startup) and installation locking live in
`administration.setup_bootstrap`, and first-run storage preparation in
`administration.setup_storage`.
