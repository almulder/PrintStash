"""Defend durable thumbnail cache ownership, leases, and publication.

Failures here risk duplicate cloud renders or pointers to objects that no
worker owns, both of which become expensive under multi-instance contention.
"""
...
class TestThumbnailGenerations:
    ...
    def test_deterministic_failure_is_negative_cached(db_session: Session) -> None:
        ...
        from app.modules.media.thumbnail_generations import _get_or_create_generation
    ...
    @staticmethod
    def test_concurrent_requests_for_one_recipe_render_once(
        db_session: Session,
    ) -> None:
        model = build_model(db_session)
        file_row = build_file(
    ...
    def test_invalid_renderer_bytes_never_publish(db_session: Session) -> None:
        model = build_model(db_session)
        file_row = build_file(
            db_session, model, filename="bad-render.stl", sha256="5" * 64
        )
        backend = get_backend()
        backend.write_bytes(b"mesh", file_row.path)
    ...
    def test_publication_exception_becomes_a_transient_storage_failure(
        ...
        import app.modules.media.thumbnail_generations as generations

        model = build_model(db_session)
        file_row = build_file(
            db_session, model, filename="storage.stl", sha256="8" * 64
        )
        backend = get_backend()
        backend.write_bytes(b"mesh", file_row.path)
        monkeypatch.setattr(
